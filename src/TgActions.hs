...
checkIfAdmin tok uid cid = do
    res_chat <- reqSend tok "getChat" $ GetChatAdministrators cid
    let chat_resp = responseBody res_chat :: TgGetChatResponse
        c = resp_result chat_resp :: Chat
    if chat_type c == Private then pure True else do
        resp_chat_admins <- reqSend tok "getChatAdministrators" $ GetChatAdministrators cid
        let res_cms = responseBody resp_chat_admins :: TgGetChatMembersResponse
