...
import AppTypes (App, AppConfig (last_worker_run, subs_state, tasks_queue, tg_config, worker_interval), FeedsAction (IncReadsF, RefreshNotifyF), FeedsRes (FeedBatches), Job (IncReadsJob, TgAlert, Log), ServerConfig (alert_chat, bot_token), SubChat (sub_chatid, sub_last_notification), db_config, runApp, DbAction (UpsertChats))
            ...
            runApp (env {last_worker_run = Just now}) $ evalFeedsAct RefreshNotifyF >>= \case
                FeedBatches p -> liftIO $ do
                    ...
                    modifyMVar_ (subs_state env) $ \subs -> 
                        ...
                        in  runApp env $ evalMongoAct config (UpsertChats updated_chats) >> pure updated_chats
