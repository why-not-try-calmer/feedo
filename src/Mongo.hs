...
primaryOrSecondary rep = try (primary rep) >>= \case
    Left (_ :: SomeException) -> try (secondaryOk rep) >>= \case
...
initConnectionMongo creds@MongoCredsReplicaTls{..} = liftIO $ do
    ...
    maybe   (pure . Left $ PipeNotAcquired)
            (\p -> isClosed p >>= \v -> if v then pure . Left $ PipeNotAcquired else login creds p) mb_pipe
...
evalMongo env (UpsertPages cid mid pages mb_link) =
