...
parse = runParser . Parser $ \t -> runner . T.reverse $ t
 where
  ...
  step _ (Right (FailOn err, count)) =
    Left $
      "Found error at character "
...
parsing (Rep Open (ltags, rtags) (child : children)) c = case parsing child c of
  ...
  Back ->
    if not (ltags `closeOn` c)
      then Up $ Rep Open (ltags, rtags) (mkEntity c : child : children)
      else
        let rtags' = c : rtags
         in if ltags == rtags'
              then Up $ Rep Closed (ltags, rtags') (child : children)
...
parsing (NonRep Open tag (child : children)) c = case parsing child c of
  ...
  Back ->
    if [tag] `closeOn` c
      then Up $ NonRep Closed tag (child : children)
