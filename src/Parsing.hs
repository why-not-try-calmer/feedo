...
buildFeed ty url = do
    ...
    faultyFeed f =
        let holes = [T.null $ f_desc f, T.null $ f_title f, T.null $ f_link f, null . f_items $ f]
            ...
            render_required = "The required feed could be constructed, but it's missing well-defined tags or items: "
            ...
            render_optional = if null missing_optional then Nothing else Just $
                "A valid feed could be constructed. However, notice that the following fields could not be \
                \ set appropriately: " `T.append` T.intercalate ", " missing_optional
        in  if null missing_required then Right (f, render_optional)
...
rebuildFeed key = case eitherUrlScheme key of
    ...
    Right url -> buildFeed Rss url >>= \case
        Left _ -> buildFeed Atom url >>= \case
            Left build_error -> pure . Left $ renderUrl url `T.append` " ran into this error: "
